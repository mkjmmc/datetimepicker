  ...
  Datapicker.prototype = {
    ...
    fill: function () {
      ...
      if(this.isSelect){
		  ...
		  var $scroller = this.picker.find('.picker-scroller');
		  var $clone = this.picker.find('.clone-scroller');
		  
		  // 填充数据
		  $clone.html(html.join(''));
		  $scroller.html(html.join(''));

		  // 计算角度
		  var unit = this.deg / this.lineHeight;
		  unit = Math.round(unit/this.deg)*this.deg;
		  transform($scroller[0], "translateZ(-90px) rotateX(" + unit + "deg)")
		  ...
		  $scroller.find(".option").each(function (index, $option) {
            $option = $($option);
